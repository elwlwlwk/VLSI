...
public class ThreeOptSearch {
	...
	public static int[] swap(int[] arr){
		...
		for(int trial = 0; trial < trial_max; trial++){
			int[] idx = {(int) (random.nextFloat() * (best_path.length)),
					(int) (random.nextFloat() * (best_path.length)),
					(int) (random.nextFloat() * (best_path.length))};
			Arrays.sort(idx);
			if(idx[0] >= best_path.length
					|| idx[1] >= best_path.length
					|| idx[2] >= best_path.length){
				continue;
			}
